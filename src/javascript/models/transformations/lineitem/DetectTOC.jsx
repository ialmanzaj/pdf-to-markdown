...
export default class DetectTOC extends ToLineItemTransformation {
  ...
  transform(parseResult : ParseResult) {
    ...
    if (tocPages.length > 0) {
      ...
      messages.push('Found TOC headlines: ' + (
      foundHeadlines - notFoundHeadlines.length + foundBySize.length) + '/' + foundHeadlines);
    }
    if (notFoundHeadlines.length > 0) {
...
function findHeadlineItems(page, headline) {
  ...
  for (var line of page.items) {
    ...
    if (headlineItems) {
      return {lineIndex: lineIndex, headlineItems: headlineItems};
    }
    lineIndex++;
  }
  return null;
